...
namespace NextCloud
{
    ...
    public class Share : ApiEntryBase
    {
        ...
        public string public_preview { get; set; }
        public string public_href { get; set; }


        static public async Task<Share> Get(NextCloudService api, string shareid)
        {
            OcsShareEntry entry = await api.GetAsync<OcsShareEntry>(NextCloudService.Combine("ocs/v2.php/apps/files_sharing/api/v1/shares", shareid));
            return entry.ocs.data.FirstOrDefault().ConvertToObject<Share>();
        }

        static public async Task<PlainList<OcsShareEntry>> List(NextCloudService api)
        {
            return await api.GetPlainListAsync<OcsShareEntry>("ocs/v2.php/apps/files_sharing/api/v1/shares", "ocs.data");
        }

        static public async Task<Share> Create(NextCloudService api, ShareCreateInfo info)
        {
            ...
            return r.ocs.data.ConvertToObject<Share>();
        }

        public async Task Update(NextCloudService api, Share share)
        {
            var info = new
            {
            ...
            await api.PutAsync(NextCloudService.Combine("ocs/v2.php/apps/files_sharing/api/v1/shares", share.id), null, info);
        }




        static public async Task Delete(NextCloudService api, string shareid)
        {
            await api.DeleteAsync(NextCloudService.Combine("ocs/v2.php/apps/files_sharing/api/v1/shares", shareid));
        }
