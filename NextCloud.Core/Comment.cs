...
namespace NextCloud {
	...
	public class CommentList : ApiList<Comment> {
		...
		public override async Task<ApiList<Comment>> Read(NextCloudService api) {
			XElement data = await api.SendMessageAsyncAndGetXmlResponse(NextCloudService.REPORT, MetaData.Uri, PostParams());
			return Convert(data);
		}
	...
	public class Comment : ApiEntryBase {
		...
		public static async Task Create(NextCloudService api, string fileId, string message) {
			await api.SendMessageAsyncAndGetStringResponse(HttpMethod.Post, NextCloudService.Combine("remote.php/dav/comments/files", fileId), 
				new {
					actorType = "users",
					message,
					objectType = "files",
		...
		public static async Task<ApiList<Comment>> List(NextCloudService api, string fileId, ListRequest request = null) {
			CommentList r = new CommentList() {
				Request = request ?? new ListRequest(),
				MetaData = new MetaData() {
					Uri = NextCloudService.Combine("remote.php/dav/comments/files", fileId)
				}
			};
