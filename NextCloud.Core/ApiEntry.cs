...
namespace NextCloud {
	...
	public class ApiList<T> : ApiList {
		...
		/// </summary>
		public async Task<ApiList<T>> GetNext(NextCloudService api) {
			if (!HasMoreData)
				return null;
		...
		public virtual async Task<ApiList<T>> Read(NextCloudService api) {
			JObject j = await api.SendMessageAsync(Request.PostParameters == null ? HttpMethod.Get : HttpMethod.Post,
							NextCloudService.AddGetParams(MetaData.Uri, new {
								Request.search,
								Request.offset,
								Request.limit
		...
		/// <returns></returns>
		public IEnumerable<T> All(NextCloudService api) {
